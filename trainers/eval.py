...
def meta_test(data_path,model,way,shot,pre,transform_type,query_shot=16,trial=10000,return_list=False,epoch=None, hw_range=None):
    ...
    for i, (inp,_) in tqdm(enumerate(eval_loader)):
        ...
        inp = inp.cuda()
        max_index = model.meta_test(inp,way=way,shot=shot,query_shot=query_shot,epoch=epoch,hw_range=hw_range)
